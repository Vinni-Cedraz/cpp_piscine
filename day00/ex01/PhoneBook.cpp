...
void PhoneBook::search_CMD() {
    ...
    display_contacts();
    while (true) {
        std::cout << CYAN "Enter the index of the contact you want to search: (0-" << this->idx - 1 << "): " RESET;
        std::cin >> index;
        if (std::cin.eof())
            exit(1);
...
void PhoneBook::search_contact(const int index) {
    if (index > 7 || contacts[index].getUserInput().first_name.empty())
        std::cerr << RED "Error: out of range index." RESET;
    std::cout << "First Name: " << contacts[index].getUserInput().first_name << std::endl;
    std::cout << "First Name: " << contacts[index].getUserInput().last_name << std::endl;
    std::cout << "First Name: " << contacts[index].getUserInput().nickname << std::endl;
    std::cout << "First Name: " << contacts[index].getUserInput().phone_number << std::endl;
    std::cout << "First Name: " << contacts[index].getUserInput().darkest_secret << std::endl;
}
