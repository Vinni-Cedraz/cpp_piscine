...
static int *randomNumbers(size_t size) {
  ...
  for (size_t i = 0; i < size; i++) numbers[i] = rand() % 100;
  return (numbers);
}
...
int main(void) {
  ...
  std::cout << std::endl;
  {
    ...
    MutantStack<int> mstack;
    for (size_t i = 0; i < size; i++) mstack.push(numbers[i]);
